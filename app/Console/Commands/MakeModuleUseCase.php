...
class MakeModuleUseCase extends Command
{
     ...
     public function handle(): void
    {
        ...
        class {$name}
        {
            ...
            {
                return \$this->repository->{$this->resolveRepositoryMethod($name)}(\$dto);
            }
        }
        PHP;
    ...
    private function resolveRepositoryMethod(string $name): string
    {
        ...
        if (Str::startsWith(strtolower($verb), 'create')) return 'create';
        if (Str::startsWith(strtolower($verb), 'update')) return 'update';
        if (Str::startsWith(strtolower($verb), 'delete')) return 'delete';
        if (Str::startsWith(strtolower($verb), 'get') || Str::startsWith(strtolower($verb), 'find')) return 'findById';
        if (Str::startsWith(strtolower($verb), 'list') || Str::startsWith(strtolower($verb), 'get all')) return 'getAll';
