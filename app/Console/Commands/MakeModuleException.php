...
class MakeModuleException extends Command
{
    ...
    private function resolveHttpStatus(string $name): int
    {
        ...
        if (str_contains($lower, 'notfound') || str_contains($lower, 'not_found')) return 404;
        if (str_contains($lower, 'unauthorized'))                                   return 401;
        if (str_contains($lower, 'forbidden'))                                      return 403;
        if (str_contains($lower, 'invalid') || str_contains($lower, 'validation'))  return 422;
        if (str_contains($lower, 'conflict') || str_contains($lower, 'duplicate'))  return 409;
