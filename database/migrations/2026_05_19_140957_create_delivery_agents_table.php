...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('delivery_agents', function (Blueprint $table) {
            ...
            $table
                ->foreignUuid('supervisor_agent_id')
                ->nullable()
                ->references('delivery_agent_id')
                ->on('delivery_agents')
                ->onDelete('set null')
                ->comment('NULL = this agent is a supervisor; NOT NULL = points to his supervisor');
            $table
            ...
            $table
                ...
                ->default(0)
                ->comment('Signed unsettled position: positive=agent owes system, negative=system owes agent');
            $table
                ->tinyInteger('is_available')
                ->default(1)
        ...
        Schema::create('staff_members', function (Blueprint $table) {
            $table->uuid('staff_member_id')->primary();
            $table
                ->foreignUuid('user_id')
                ->references('user_id')
                ->on('users')
            ...
            $table
                ->text('notes')
                ->nullable()
                ->comment('Internal notes about this staff member');
            $table->softDeletes();
            $table->timestamps();
        });
    }
